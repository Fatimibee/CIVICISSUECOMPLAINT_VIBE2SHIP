...
st.set_page_config(
    page_title="Civic Shield Admin Dashboard", page_icon="🏢", layout="wide"
)
...
with col1:
    ...
    user_email = st.text_input(
        "Citizen Email Address", placeholder="e.g., citizen@email.com"
    )
    location = st.text_input(
        "Incident Location / Landmark", placeholder="e.g., Ward No. 4, SKIT Campus"
    )
    uploaded_file = st.file_uploader(
        "Upload Infrastructure Evidence (Image)", type=["jpg", "jpeg", "png"]
    )
...
with col2:
        ...
        if not user_email or not location or not uploaded_file:
            ...
            with st.spinner("Uploading binary image and streaming agent nodes..."):
                try:
                    ...
                    form_payload = {"userEmail": user_email, "location": location}
                    ...
                    if response.status_code == 200:
                        ...
                        if status == "IMAGE_TOO_UNCLEAR":
                            ...
                            st.session_state.current_draft = res_data.get(
                                "emailDraft", ""
                            )
                            st.success(
                                f"🟢 Classification complete: Recognized '{res_data.get('issue')}' layout context."
                            )
                    else:
                        st.error(
                            f"Server integration failure code: {response.status_code}"
                        )
                        st.json(
                            response.json()
                        )  # Inspect validation complaints if any remain
    ...
    if st.session_state.thread_id:
        ...
        editable_draft = st.text_area(
            "Review / Tweak Department Email Payload",
        ...
        with review_col1:
            if st.button("✅ Approve & Dispatch Email", type="primary"):
                with st.spinner("Confirming graph execution state changes..."):
                    ...
                    res = requests.post(
                        f"{BACKEND_URL}/human_action", json=action_payload
                    )
                    if res.status_code == 200:
                        st.balloons()
                        st.success(
                            "🎉 Transaction Completed! Email sent and database logs updated successfully."
                        )
                        st.session_state.thread_id = None
                        st.session_state.current_draft = ""
                    else:
        ...
        with review_col2:
            ...
            if st.button("🔄 Request Regeneration"):
                if not feedback:
                    st.warning(
                        "Please specify modification parameters before executing regeneration routing."
                    )
                else:
                    with st.spinner("Rewriting dynamic state metrics..."):
                        ...
                        if res.status_code == 200:
                            st.info(
                                "🔄 Suggestion applied! (Note: Refresh graph stream to pull updated model output text layout)"
                            )
